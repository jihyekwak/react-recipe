...
function Edit(props) {
  ...
  return(<div className='edit-post'>
    <form className='create-form' onSubmit={(e) => {
      ...
      <input type="text" name="title" placeholder='what do you make' value={title} required onChange={(e)=>setTitle(e.target.value)}></input>
      <input type="text" name="ingredients" placeholder='what do you need' value={ingredients} required onChange={(e)=>setIngredients(e.target.value)}></input>
      <textarea type="text" name="body" placeholder='how do you make' value={body} required onChange={(e)=>setBody(e.target.value)}></textarea>
      <input type="url" name="imgUrl" placeholder='image url' value={imgUrl} onChange={(e)=>setImgUrl(e.target.value)}></input>
      <input type="submit"></input>
    </form>
  </div>)
}
      ...
      {(mode === 'CREATE') ?
        <>
          ...
          <Create onCreate={(_title, _ingredients, _body, _imgUrl) => {
            ...
            const newRecipe = {id: nextId, title: _title, ingredients: ingredients, body: _body, imgUrl: _imgUrl};
        ...
        : (mode === 'DETAIL') ? <>
          ...
          <Edit recipe={detailRecipe} onEdit={(_title, _ingredients, _body, _imgUrl) => {
            const editedIngredients = _ingredients.split(',');
            const editedRecipe = Object.assign({detailRecipe, title: _title, ingredients: editedIngredients, body: _body, imgUrl: _imgUrl })
            const newRecipes = recipes.map(recipe => recipe === detailRecipe ? editedRecipe : recipe);
            setRecipes(newRecipes); 
